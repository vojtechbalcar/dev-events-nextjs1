...
const EventDetailsPage = async  ( { params} : { params : Promise<{slug : string}>} ) => {
    ...
    const { event : {_id, description, image, overview, date, time, location, mode, agenda, audience, tags, organizer, slug: eventSlug } } = await request.json();
            ...
            <div className="flex w-full flex-col gap-4 pt-20">
                ...
                <div className="events">
                    {similiarEvents.length > 0 && similiarEvents.map((similiarEvent : IEvent) => (
                        <EventCard key={similiarEvent._id}{ ...similiarEvent } />
                    ))}
                </div>
            </div>
        </section>
